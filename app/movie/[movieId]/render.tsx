...
import { saveMovieReview, getMovieReviewById, getMovieById, getSavedMovies } from "@/utils";
...
const MoviePageRender: React.FC<MoviePageRenderProps> = ({ movie }) => {
  ...
  const [isRatingEditable, setIsRatingEditable] = useState(true); // Add state for rating editability
  ...
  useEffect(() => {
    ...
    if (savedMovie) {
      ...
      setIsSaved(true);
      setIsRatingEditable(false); // Disable rating when the review is saved
    }
  }, [movie.id]);
  ...
  const handleSaveReview = () => {
    ...
    if (existingReview) {
      ...
      const index = savedReviews.findIndex((item: SavedMovie) => item.id === movie.id);
